...
def remap_one(
    ...
    tri_indexs, ray_indexs, locations = obj.ray.intersects_id(
        origin, direction, return_locations=True, multiple_hits=False
    )  # N
    hh = hh[ray_indexs]
    ww = ww[ray_indexs]
    ...
    if to_vers:
        ...
        if "neardist" in obj.vertex_attributes:
            ...
            colors = colors[valid]
            obj.vertex_attributes["cate_colors"][nearvers, :][valid, :] = colors[
                valid, :
            ]
        else:
            obj.vertex_attributes["neardist"] = np.ones(len(obj.vertices)) * np.inf
            obj.vertex_attributes["cate_colors"] = np.zeros(
                (len(obj.vertices), img.shape[2])
            )
...
def merge_remap(obj: trimesh.Trimesh):
    ...
    obj.visual = visual.ColorVisuals(
        obj, face_colors=faces_colors, vertex_colors=vers_colors
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--depth", default=False, action="store_true", help="是否保存深度信息"
    )
    parser.add_argument(
        "--dsm", default=False, action="store_true", help="是否保存DSM信息"
    )
    parser.add_argument(
        "--bgcolor", type=float, nargs=4, default=(0.0, 0.0, 0.0, 0), help="背景颜色"
    )
    ...
    cameras = parser_ccxml(
        args.export_xml, os.path.join(args.data_root, "..", "metadata.xml")
    )
