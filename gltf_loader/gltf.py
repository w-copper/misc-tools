    ...
    for index, m in enumerate(header.get("meshes", [])):
        try:
            ...
            for p in m["primitives"]:
                ...
                if 'KHR_draco_mesh_compression' in p['extensions']:
                    ext_dict = p['extensions']['KHR_draco_mesh_compression']
                    khrdecode = DracoPy.decode(views[ext_dict['bufferView']])
                    khrattr = ext_dict['attributes']
                    if 'POSITION' in khrattr:
                        kwargs['faces'] = khrdecode.faces
                        kwargs['vertices'] = khrdecode.points
                    if 'NORMAL' in khrattr:
                        kwargs['vertex_normals'] = khrdecode.normals
                    if 'TEXCOORD_0' in khrattr and 'material' in p:
                        uv = khrdecode.tex_coord
                        uv = uv.copy()
                        uv[:, 1] = 1 - uv[:, 1]
